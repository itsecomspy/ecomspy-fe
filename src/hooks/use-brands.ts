...
export default function useBrands({
  ...
  const getBrandsMutation = useMutation<
    {
      query: string;
      value: number;
      volume: number;
  ...
  >({
    mutationKey: ["brands", categoryId || values?.child],
    mutationFn: async () => {
      let id = categoryId;
      ...
      const res = await firebaseService.callFunction("getCategoryBrands", {
        category: id,
  ...
  useEffect(() => {
    getBrandsMutation.mutate();
  }, [filterAtomData]);

  return {
    brands: getBrandsMutation.data,
    isLoading: getBrandsMutation.isPending,
  };
}
