...
export const AuthProvider = ({ children }: React.PropsWithChildren) => {
  const { retreiveSubscription } = useStripe();
  ...
  const register = React.useCallback(
    async ({ userData }: { userData: UserDataProps }) => {
      ...
      return firebaseService
        ...
        .then(async (res) => {
          firebaseService.setDocumentWithId(res.user.uid, "users", userData);
          setUser(res.user);
        ...
        .then((res) => {
          ...
          if (0 === 1) {
            retreiveSubscription({
              ...
              .then((res) => {
                setSubscriptionData({
                  cancelled: !!res.cancel_at_period_end,
                  endDate: !!res.cancel_at_period_end
                    ? res.cancel_at
                    : res.current_period_end,
                  status: !!res.cancel_at_period_end ? "cancelled" : "active",
